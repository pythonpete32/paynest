...
contract PaymentsPluginSetupTest is Test {
    ...
    function test_prepareInstallation_ShouldDeployPluginProxySuccessfully()
        public
    {
        ...
        (address plugin, ) = pluginSetup.prepareInstallation(
            address(dao),
            installationParams
        );
        ...
        assertEq(
            address(PaymentsPlugin(plugin).llamaPayFactory()),
            address(0x456)
        );
    }

    function test_prepareInstallation_ShouldGrantMANAGER_PERMISSIONToManagerAddress()
        public
    {
        bytes memory installationParams = pluginSetup.encodeInstallationParams(
            alice,
        ...
        assertEq(
            preparedSetupData.permissions[0].permissionId,
            keccak256("MANAGER_PERMISSION")
        );
    }

    function test_prepareInstallation_ShouldGrantEXECUTE_PERMISSIONToPluginOnDAO()
        public
    {
        bytes memory installationParams = pluginSetup.encodeInstallationParams(
            alice,
        ...
        assertEq(
            preparedSetupData.permissions[1].permissionId,
            dao.EXECUTE_PERMISSION_ID()
        );
    }

    function test_prepareInstallation_ShouldRevertWithInvalidManagerAddressForZeroManager()
        public
    {
        bytes memory installationParams = pluginSetup.encodeInstallationParams(
            address(0),
    ...
    function test_prepareInstallation_ShouldRevertWithInvalidRegistryAddressForZeroRegistry()
        public
    {
        bytes memory installationParams = pluginSetup.encodeInstallationParams(
            alice,
    ...
    function test_prepareInstallation_ShouldRevertWithInvalidLlamaPayFactoryForZeroFactory()
        public
    {
        bytes memory installationParams = pluginSetup.encodeInstallationParams(
            alice,
    ...
    function test_prepareUninstallation_ShouldRevokeMANAGER_PERMISSIONFromManagerAddress()
        public
    {
        // First install
        bytes memory installationParams = pluginSetup.encodeInstallationParams(
        ...
        // Test uninstallation
        PermissionLib.MultiTargetPermission[] memory permissions = pluginSetup
        ...
        assertEq(
            uint256(permissions[0].operation),
            uint256(PermissionLib.Operation.Revoke)
        );
    }

    function test_prepareUninstallation_ShouldRevokeEXECUTE_PERMISSIONFromPluginOnDAO()
        public
    {
        // First install
        bytes memory installationParams = pluginSetup.encodeInstallationParams(
        ...
        assertEq(
            uint256(permissions[1].operation),
            uint256(PermissionLib.Operation.Revoke)
        );
    }
    ...
    function test_encodeInstallationParameters_ShouldEncodeParametersCorrectly()
        public
        view
    {
        bytes memory encoded = pluginSetup.encodeInstallationParams(
            alice,
            address(0x123),
            address(0x456)
        );
    ...
    function test_decodeInstallationParameters_ShouldDecodeParametersCorrectly()
        public
        view
    {
        bytes memory encoded = pluginSetup.encodeInstallationParams(
        ...
        (address manager, address registry, address factory) = pluginSetup
            .decodeInstallationParams(encoded);
    ...
    function test_decodeInstallationParameters_ShouldMatchOriginalValues()
        public
        view
    {
        ...
        (
            address decodedManager,
            address decodedRegistry,
            address decodedFactory
    ...
    function test_encodeUninstallationParameters_ShouldEncodeManagerAddressCorrectly()
        public
        view
    {
    ...
    function test_decodeUninstallationParameters_ShouldDecodeManagerAddressCorrectly()
        public
        view
    {
        bytes memory encoded = pluginSetup.encodeUninstallationParams(alice);
    ...
    function test_getImplementation_ShouldReturnCorrectImplementationAddress()
        public
        view
    {
